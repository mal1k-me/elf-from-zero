...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        if (strncmp(argv[i], TARGET_FLAG, TARGET_FLAG_LEN) == 0) {
            requested_triple = argv[i] + TARGET_FLAG_LEN;
        } else {
            print_usage(argv[0]);
            return 1;
        }
    ...
    if (!requested_triple) {
        ...
        if (!default_triple) {
            (void)fprintf(stderr, "Failed to determine host target triple\n");
            return 1;
        }
    ...
    if (!triple_copy) {
        (void)fprintf(stderr, "Failed to copy target triple string\n");
        if (default_triple) {
    ...
    if (!config) {
        (void)fprintf(stderr,
                      ...
                      "Ensure include/generated_arch_config.h contains entries "
                      "for "
                      "this target.\n",
                      triple_copy);
        free(triple_copy);
    ...
    (void)printf("Detected Architecture: %s\n", config->keyword);
    (void)printf("Target Triple:         %s\n", triple_copy);
